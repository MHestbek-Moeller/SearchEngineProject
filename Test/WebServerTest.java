...
*/
    ...
    void setUp() {
        try {
            ...
            while (server == null) {
                try {
                    q=new AdvancedQueryEngine("data/test-file.txt");
                    server = new WebServer(rnd.nextInt(60000) + 1024, q);
...
/**
 * Tears down the server so it does not keep running once tests are executed
 */
    @AfterAll
    void tearDown() {
